...
const ServiceCard: React.FC<{ service: ServiceDisplay; viewMode: 'grid' | 'list' }> = ({ service, viewMode }) => {
    ...
    return (
        <div className={`relative bg-white dark:bg-custom-purple-surface rounded-xl shadow-lg hover:shadow-xl transition-all duration-300 border border-zinc-100 dark:border-zinc-800 group animate-fadeIn
            ${isList 
        ...
        >
            ...
            {/* Ikon Layanan */}
            <div className={`flex-shrink-0 text-custom-purple-primary dark:text-custom-purple-light transition-transform duration-300
                ${isList 
            ...
            {/* Konten Teks */}
            <div className={`flex-grow ${isList ? 'w-full md:w-auto' : 'text-center'}`}>
                <span className={`inline-block px-3 py-1 mb-2 text-xs font-medium rounded-full bg-indigo-100 text-indigo-800 dark:bg-indigo-900 dark:text-indigo-200
                    ${isList ? '' : 'mb-3'}
                `}>
                    {service.category}
                </span>
                <h3 className={`text-xl font-bold text-zinc-800 dark:text-zinc-100 mb-2 ${isList ? '' : 'mb-3 line-clamp-2'}`}>
                    {service.title}
                </h3>
                <p className={`text-zinc-500 dark:text-zinc-300 text-sm ${isList ? 'mb-0' : 'mb-6'}`}>
                    {service.description}
                </p>
            </div>
            
            {/* Tombol CTA */}
            <div className={`mt-4 pt-0 ${isList ? 'md:mt-0 md:ml-6 w-full md:w-auto flex-shrink-0' : 'mt-auto w-full pt-4'}`}>
                <button 
                    onClick={handleOrder}
                    className={`flex items-center justify-center py-2.5 px-4 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg transition-all duration-300 shadow-md hover:shadow-lg focus:ring-2 focus:ring-green-400 focus:ring-offset-2 dark:focus:ring-offset-custom-purple-surface transform active:scale-95
                        ${isList ? 'w-full md:w-auto whitespace-nowrap' : 'w-full'}
                    `}
                >
                    <WhatsAppIconSmall />
                    Pesan Sekarang
...
const Services: React.FC = () => {
    ...
    useEffect(() => {
        const fetchServices = async () => {
            try {
                ...
                const uniqueCategories = ['Semua', ...new Set(formattedData.map(item => item.category))];
    ...
    return (
        <section className="py-20 bg-zinc-50 dark:bg-custom-purple-bg min-h-screen transition-colors duration-300">
            <div className="container mx-auto px-6">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4">
                            ...
                            <div className="flex bg-white dark:bg-custom-purple-surface rounded-xl border border-zinc-200 dark:border-zinc-700 p-1 shadow-sm flex-shrink-0 self-start sm:self-auto">
                                <button
                                    ...
                                    className={`p-2 rounded-lg transition-colors ${viewMode === 'grid' ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900 dark:text-indigo-200' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`}
                                ...
                                <button
                                    ...
                                    className={`p-2 rounded-lg transition-colors ${viewMode === 'list' ? 'bg-indigo-100 text-indigo-600 dark:bg-indigo-900 dark:text-indigo-200' : 'text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-300'}`}
                ...
                {loading ? (
                    ...
                    <div className="text-center py-12 bg-white dark:bg-custom-purple-surface rounded-xl shadow-sm max-w-2xl mx-auto border border-zinc-100 dark:border-zinc-800">
                        ...
                        <h3 className="text-lg font-medium text-zinc-900 dark:text-zinc-100">Layanan tidak ditemukan</h3>
                        <button 
